...
- `Aurelian.Graphics` now owns the first graphics HAL scaffold, native-free plant registry, and Vulkan instance/device initialization M0 for plant 0. `PlantId.Zero` represents the single-GPU plant in M0; successful Vulkan creation is per-plant, requires timeline semaphores, records plain facts/diagnostics, and keeps window/surface/swapchain/resources deferred.
