...
A66 updates the human-facing visible triangle sample to exercise the current engine spine: sample-owned prepared Vulkan setup feeds `AurelianEngine`, a started Dominatus-backed `AurelianRuntimeSession`, `AurelianRuntimeTickFrameStep`, `AurelianFrameLoop`, runtime tick, frame pump, runtime compositor policy, Core compositor bridge, Vulkan compositor mechanism, and presentation. The sample remains finite and one-frame by default because it still owns one prepared swapchain acquire/target for M0; multi-frame acquire/present, asset loading, runtime shader compilation, full game-loop hosting, and engine-owned graphics lifecycle remain deferred.
