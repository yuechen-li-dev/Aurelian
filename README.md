...
A68 updates the human-facing visible triangle sample into a minimal finite desktop-app shape: sample-owned prepared Vulkan setup feeds `AurelianEngine`, a started Dominatus-backed `AurelianRuntimeSession`, `AurelianRuntimeTickFrameStep`, and `AurelianFrameLoop`; the sample-local input provider pumps window events before each swapchain acquire, acquires per-frame images, and stops cleanly when Silk.NET reports a close request; the sample-local presentation mechanism presents the acquired image and pumps events again after present. The loop remains finite (`--frames`, default `3`, capped at `300`) and there is still no production host, production input system, engine-owned window lifecycle, asset loading, runtime shader compilation, or full game loop.
