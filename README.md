...
A13 implements World Unit M0 under `Aurelian.World.Units`: IDs, descriptors, immediate-only composition, a simple resolver, resolved parent/child maps, pre-order traversal, transitive descendant queries, and diagnostics for missing roots, missing children, duplicate immediate children, duplicate child slots, and composition cycles. A14 adds `Aurelian.Actuation.World` as the first typed mutation boundary over those documents: requests are pure data, results always carry a `WorldDocument`, rejected/no-op operations keep the original document, applied operations return a new document, and the resolver validates applied mutations. The world project remains data/composition/resolution only and does not reference actuation, Dominatus, rendering, assets, or shaders. M0 deliberately does not introduce a full ECS, processors, an entity manager, behavior runtime, renderer integration, asset integration, physics, LLM calls, deep inheritance, or blackboards; blackboards remain a logic/runtime concern rather than durable world data. A15 adds the dependency policy that should guide A16 World typed data stores M0 and later actuation/render contract work: keep `Aurelian.World` library-free, use external libraries only behind contracts, and keep core behavior reflection-free by default.
