...
## A44 compiled shader program graphics pipeline note

A44 implements the graphics-side bridge from neutral `CompiledShaderProgram` contracts to Vulkan graphics pipeline descriptors. `Aurelian.Graphics` validates the M0 graphics shader shape (one vertex stage, one fragment stage, no compute stages, no duplicate stages), reuses `VulkanCompiledShaderStageMapper`, preserves caller-supplied vertex input descriptors, and can optionally call the existing native Vulkan graphics pipeline factory when a plant and render pass are available.

The dependency boundary remains unchanged: `Aurelian.Graphics` consumes `Aurelian.Rendering.Contracts.Shaders` only, does not reference `Aurelian.Shaders`, does not invoke DXC, and does not know about SDSL-V compiler artifacts. Asset/TOML integration, descriptor sets, uniforms, pipeline bind commands, draw commands, and swapchain/window/surface work remain deferred.

## A45 pipeline bind + vertex draw command M0 status

A45 adds the first validated Vulkan vertex draw command path in `Aurelian.Graphics`. Successful render pass begin now returns a typed `VulkanRenderPassScope`, and `VulkanCommandBufferLease` owns minimal active render-pass state so draw/end commands validate the provided scope against the actual command buffer lease rather than relying on Vulkan validation layers.

`VulkanDrawCommandEncoder.DrawVertices(...)` validates command-buffer recording state, active render pass scope, plant ownership, pipeline lifetime, one vertex buffer lifetime/`Vertex` usage, positive vertex count, and viewport/scissor bounds before recording `vkCmdSetViewport`, `vkCmdSetScissor`, `vkCmdBindPipeline`, `vkCmdBindVertexBuffers`, and a non-indexed `vkCmdDraw` with one instance. A45 deliberately adds no swapchain/window/surface, presentation, descriptor sets, uniform buffers, push constants, index buffers, multiple vertex buffers, pipeline cache, shader compiler dependency, or `RenderCommandPlan` execution.

## A46 valid SPIR-V offscreen draw recording proof
...
## A54 First visible triangle through compositor path status

A54 connects the existing offscreen triangle draw proof, compositor passthrough copy mechanism, and swapchain acquire/present seam in one unavailable/headless-safe integration test. When presentation is available, the test creates a presentation-enabled Vulkan plant, acquires a swapchain image, renders the checked-in SPIR-V triangle fixture into an offscreen color target, wraps that offscreen texture as a `PlantOutputRef`, dispatches `CompositorPolicyKind.Passthrough` into the acquired `PresentationTargetRef`, and presents the image.

The visible path intentionally goes through compositor passthrough instead of rendering directly to the swapchain. A54 also keeps render-pass attachment layout tracking honest after `vkCmdEndRenderPass`, so the offscreen source texture is tracked at the render pass final layout before compositor barriers run. There is still no frame loop, renderer facade, Dominatus compositor policy, differential compositor, shader compiler dependency in graphics, VMA/VMASharp, or Vortice dependency.

## A55 Runtime Dominatus compositor policy M0 status
...
## A67 visible triangle finite multi-frame acquire/present status
...
A71 updates `samples/Aurelian.VisibleTriangle` so its startup path loads `Assets/assets.toml` through `ShaderAssetManifestLoader`, selects shader id `smoke_triangle`, and passes the loaded neutral `CompiledShaderProgram` into the existing Vulkan graphics pipeline setup. Direct C# loading of `Assets/Shaders/SmokeTriangle/shader.toml` is removed; the artifact path now lives behind the asset manifest entry.

A71 is sample asset-manifest consumption only. It does not add a material system, mesh asset system, texture asset system, general asset manager/cache/hot reload, runtime shader compilation, DXC/SDSL runtime dependencies, `Aurelian.Host`, or new package dependencies. `Aurelian.Assets` remains graphics-free and `Aurelian.Graphics` remains asset/shader-loader-free.
