...
A69 adds the first real file-based shader artifact bridge: `Aurelian.Shaders` can write TOML metadata plus SPIR-V byte files and optional `generated.hlsl`, `Aurelian.Assets` can load and hash-check that artifact shape into neutral `CompiledShaderProgram`, and the visible triangle sample now loads checked-in text-safe `.spv.hex` shader artifacts from files instead of embedding sample-local C# SPIR-V arrays. A68 updated the human-facing visible triangle sample into a minimal finite desktop-app shape: sample-owned prepared Vulkan setup feeds `AurelianEngine`, a started Dominatus-backed `AurelianRuntimeSession`, `AurelianRuntimeTickFrameStep`, and `AurelianFrameLoop`; the sample-local input provider pumps window events before each swapchain acquire, acquires per-frame images, and stops cleanly when Silk.NET reports a close request; the sample-local presentation mechanism presents the acquired image and pumps events again after present. The loop remains finite (`--frames`, default `3`, capped at `300`) and there is still no production host, production input system, engine-owned window lifecycle, full asset manager, runtime shader compilation, or full game loop.
