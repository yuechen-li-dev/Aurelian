...
A65 connects the A63 Core frame loop to the A64 Dominatus-backed runtime session through a small Core-owned runtime ticker seam. `AurelianFrameLoop` can now request frame input, tick a prepared runtime session before the existing frame pump, and record the successful runtime tick result on the frame-loop iteration. Runtime behavior remains owned by `Aurelian.Runtime`; Core only orchestrates the call, Runtime remains graphics-free, Graphics remains runtime/Dominatus-free, no world/render extraction is introduced, and `ParallelAiWorldRunner` remains deferred behind the runner seam.
